...
// ripLyricsTrack - 只为单首 track 获取并保存歌词，不做音频下载
func ripLyricsTrack(t *task.Track, token string, mediaUserToken string) error {
    // 构造文件名（沿用项目的 SongFileFormat 但不带媒体后缀）
    songName := strings.NewReplacer(
        "{SongId}", t.ID,
        "{SongNumer}", fmt.Sprintf("%02d", t.TaskNum),
        "{SongName}", LimitString(t.Resp.Attributes.Name),
        "{DiscNumber}", fmt.Sprintf("%d", t.Resp.Attributes.DiscNumber),
        "{TrackNumber}", fmt.Sprintf("%d", t.Resp.Attributes.TrackNumber),
        "{Quality}", t.Quality,
        "{Tag}", "",
        "{Codec}", t.Codec,
    ...
    // 获取歌词（复用项目中已有的歌词获取函数）
    lrcStr, err := lyrics.Get(t.Storefront, t.ID, Config.LrcType, Config.Language, Config.LrcFormat, token, mediaUserToken)
    if err != nil {
        fmt.Printf("Get lyrics failed for %s: %v\n", t.ID, err)
        return err
    }

    // 确保目标目录存在
    if err := os.MkdirAll(t.SaveDir, os.ModePerm); err != nil {
        fmt.Println("Make save dir failed:", err)
        return err
    }

    // 写入歌词（使用已有 writeLyrics 函数）
    if err := writeLyrics(t.SaveDir, lrcFilename, lrcStr); err != nil {
        fmt.Printf("Write lyrics failed for %s: %v\n", t.ID, err)
        return err
    }

    fmt.Println("Lyrics downloaded:", filepath.Join(t.SaveDir, lrcFilename))
    return nil
}

// ripAlbumLyrics - 遍历专辑内每首歌，只下载歌词（顺序执行）
func ripAlbumLyrics(albumId, token, storefront, mediaUserToken string) error {
    // 注意：按项目里 ripAlbum 的用法，task.NewAlbum 接收 (storefront, albumId) 且只返回 album 对象
    ...
    // 取 manifest / metadata 用来构造保存路径
    meta := album.Resp
    artistName := LimitString(meta.Data[0].Attributes.ArtistName)
    albumName := LimitString(meta.Data[0].Attributes.Name)
    baseSaveDir := filepath.Join(Config.AlacSaveFolder, forbiddenNames.ReplaceAllString(artistName, "_"), forbiddenNames.ReplaceAllString(albumName, "_"))

    if err := os.MkdirAll(baseSaveDir, os.ModePerm); err != nil {
        fmt.Println("Make base save dir failed:", err)
        return err
    }

    // 为每首 track 设置保存目录后，只取歌词
    for i := range album.Tracks {
        album.Tracks[i].SaveDir = baseSaveDir
        // 如有需要也可设置 CoverPath: album.Tracks[i].CoverPath = filepath.Join(baseSaveDir, "cover.jpg")
        if err := ripLyricsTrack(&album.Tracks[i], token, mediaUserToken); err != nil {
            // 打印错误但继续下一个 track
            fmt.Printf("Failed to fetch lyrics for track %s: %v\n", album.Tracks[i].ID, err)
            continue
        }
        // 如需防止请求过快可在此加 sleep，例如:
...
func main() {
	...
	pflag.BoolVar(&dl_lrc, "lrc", false, "Download only lyrics (no media)")
	...
	for {
		for albumNum, urlRaw := range os.Args {
			...
			if strings.Contains(urlRaw, "/album/") {
				storefront, albumId := checkUrl(urlRaw) // 保留你现有的 store/front 解析方法
				if dl_lrc {
					err := ripAlbumLyrics(albumId, token, storefront, Config.MediaUserToken)
					if err != nil {
						fmt.Println("ripAlbumLyrics error:", err)
					}
				} else {
					err := ripAlbum(albumId, token, storefront, Config.MediaUserToken, urlArg_i)
					if err != nil {
						fmt.Println("ripAlbum error:", err)
					}
				}
				continue
